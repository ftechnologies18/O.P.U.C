...
func newGORM(dsn string, log *slog.Logger, isRuntime bool) (*gorm.DB, error) {
        gormCfg := &gorm.Config{
                ...
                // PrepareStmt désactivé : incompatible avec le pooler Neon (Supavisor)
                // en mode transaction pooling. Les prepared statements peuvent entrer
                // en conflit quand une connexion du pool réutilise un statement name
                // sur un backend différent (erreur "prepared statement name is already in use").
                // Phase 5 : ce bug est apparu quand le usecase IAM Create a commencé à
                // faire 2 opérations DB (user + auto-grant delegation) augmentant le
                // risque de conflit.
                PrepareStmt: false,
        }
