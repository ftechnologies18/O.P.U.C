...
export function RapportsView() {
  ...
  const loadRapports = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/rapports?${params.toString()}`)
      if (res.ok) {
        const data = await res.json()
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/rapports/${editRapportId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...updateData,
  ...
  async function openView(rapport: RapportListItem) {
    ...
    try {
      const res = await fetch(`/api/v1/rapports/${rapport.id}`)
      if (res.ok) {
        const data = await res.json()
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/v1/rapports/${deleteRapportId}`, {
        method: 'DELETE',
      })
