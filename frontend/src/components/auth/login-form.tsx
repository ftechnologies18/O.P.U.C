...
export function LoginForm({ onForgotPassword, onBack }: { onForgotPassword?: () => void; onBack?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Appel à l'API Go (/api/v1/auth/login → proxy vers backend :8080)
      // Le cookie httpOnly `opuc_session` est set automatiquement par le backend
      await goApi.login(email, password)
      // Login réussi → reload pour que le AuthProvider détecte la nouvelle session
      ...
      if (e instanceof ApiError) {
        if (e.status === 401) {
          ...
          setError(e.message || 'Une erreur est survenue. Veuillez réessager.')
        }
      } else {
            ...
            {process.env.NODE_ENV === 'development' && (
              <div className="mt-6 p-3 rounded-lg bg-amber-50/50 border border-amber-200/50">
                <p className="text-sm font-medium text-amber-800 mb-2">
                  Comptes de démonstration (API Go) :
                </p>
                <div className="space-y-1.5 text-sm text-amber-700">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-xs">gerant@opuc.demo</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">Chef de Projet</span>
                    <span className="font-mono text-xs">chef-projet@opuc.demo</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">Sous-traitant</span>
                    <span className="font-mono text-xs">sous-traitant@opuc.demo</span>
                  </div>
