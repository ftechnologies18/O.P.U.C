...
function TicketsTab({
  ...
  const fetchTickets = useCallback(
    async (page: number = 1) => {
      ...
      try {
        ...
        const res = await fetch(`/api/v1/support?${params.toString()}`)
        if (res.ok) {
          const data = await res.json()
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/v1/support/${deleteTarget.id}`, { method: 'DELETE' })
      if (res.ok) {
...
export function SupportView() {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const res = await fetch('/api/v1/entreprise/users')
      if (res.ok) {
        const data = await res.json()
