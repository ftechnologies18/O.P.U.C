...
export function ChantiersView() {
  ...
  const fetchChantiers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/chantiers?${params.toString()}`)
      if (res.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        res = await fetch(`/api/v1/chantiers/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        })
      } else {
        res = await fetch('/api/v1/chantiers', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        })
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/chantiers/${deletingId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
