...
export function StocksView() {
  ...
  const fetchEntrees = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/stocks/entrees?${params}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const fetchSorties = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({ chantierId: activeChantierId })
      if (stockIdFilter !== 'TOUS') params.set('stockId', stockIdFilter)
      const res = await fetch(`/api/stocks/sorties?${params}`)
      if (res.ok) {
        const data = await res.json()
  ...
  async function handleSaveMateriel() {
    ...
    try {
      ...
      if (editingMateriel) {
        const res = await fetch(`/api/stocks/${editingMateriel.id}`, {
          method: 'PUT',
  ...
  async function handleDelete() {
    ...
    setFormSubmitting(true)
    try {
      const res = await fetch(`/api/stocks/${deletingItem.id}`, {
        method: 'DELETE',
      })
      if (res.ok) {
  ...
  async function handleCreateEntree() {
    ...
    setFormSubmitting(true)
    try {
      const res = await fetch('/api/stocks/entrees', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...entreeForm,
  ...
  async function handleCreateSortie() {
    ...
    setFormSubmitting(true)
    try {
      const res = await fetch('/api/stocks/sorties', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...sortieForm,
  ...
  async function handleDeleteEntree(id: string) {
    try {
      const res = await fetch(`/api/stocks/entrees/${id}`, { method: 'DELETE' })
      if (res.ok) {
  ...
  async function handleDeleteSortie(id: string) {
    try {
      const res = await fetch(`/api/stocks/sorties/${id}`, { method: 'DELETE' })
      if (res.ok) {
