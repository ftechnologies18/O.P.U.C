...
export function EnginsView() {
  ...
  const fetchFournisseurOptions = useCallback(async () => {
    try {
      const res = await fetch('/api/sous-traitants?type=FOURNISSEUR')
      if (res.ok) {
        const json = await res.json()
  ...
  const fetchEngins = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/engins?${params.toString()}`)
      if (res.ok) {
        const json = await res.json()
  ...
  const handleSubmitEngin = async () => {
    ...
    try {
      ...
      let res: Response
      if (editingEnginId) {
        res = await fetch(`/api/engins/${editingEnginId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        })
      } else {
        res = await fetch('/api/engins', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        })
  ...
  const handleDeleteEngin = async () => {
    ...
    try {
      const res = await fetch(`/api/engins/${deletingEnginId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
  ...
  const handleSubmitLocation = async () => {
    ...
    try {
      ...
      if (editingLocationId) {
        res = await fetch(`/api/locations/${editingLocationId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        })
  ...
  const handleDeleteLocation = async () => {
    ...
    try {
      const res = await fetch(`/api/locations/${deletingLocationId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
